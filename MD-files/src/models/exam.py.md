...
| 本文件引用 | 被引用文件 | 引用类型 | 用途 |
|---|---|---|---|
| `ExamModel` | `src.models.base` | Import | 继承 `BaseJsonModel` |
...
- **返回值**：*（待补）*
- **异常行为**：无显式处理
##### `update_exam(self, eid: str, data: dict) → ...`
...
- **返回值**：*（待补）*
- **异常行为**：无显式处理
##### `get_upcoming(self, limit: int = 10) → ...`
