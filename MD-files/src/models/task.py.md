...
| 本文件引用 | 被引用文件 | 引用类型 | 用途 |
|---|---|---|---|
| `TaskModel` | `src.models.base` | Import | 继承 `BaseJsonModel` |
...
- **返回值**：*（待补）*
- **异常行为**：无显式处理
##### `update_notes(self, task_id: str, notes: str) → ...`
...
- **返回值**：*（待补）*
- **异常行为**：无显式处理
##### `update(self, task_id: str, changes: dict) → ...`
